...
# ╔═╡ 49448692-452d-4f72-93d2-4821043fa738
md"# Drug Classification"

# ╔═╡ 89ce94b2-1d21-4c52-990b-cad4a23d6527
md"## Data Exploration"

# ╔═╡ 7f86992d-243f-453f-81f4-6bd10e03c18e
drug_df = DataFrame(CSV.File("../dataset/drug200.csv", header=1))

# ╔═╡ 655e0d31-fe71-4ae3-8cbe-b10133333b94
data(drug_df) * mapping(:Age, stack=:Drug, color=:Drug) * histogram(bins=range(0,60, 40)) |> draw()

# ╔═╡ ec6cf2b7-1277-4a4f-bb9f-e518e7949115
schema(drug_df) |> DataFrames.DataFrame

# ╔═╡ 1e248458-f983-4e8e-aa44-c46992a68fba
describe(drug_df)

# ╔═╡ 109e0510-b795-4993-bbf0-46356953c389
coerce!(drug_df, :Age => Continuous)

# ╔═╡ 995030c1-7718-4859-8f8b-7a0b5246151e
coerce!(drug_df, Textual => Multiclass)

# ╔═╡ 14082773-4a88-49e9-9f10-3ca8d7589139
coerce!(drug_df, :Drug => OrderedFactor)

# ╔═╡ aba66416-8c4a-4d70-aa50-a6642350481f
schema(drug_df) |> DataFrames.DataFrame

# ╔═╡ 9efe4a53-e548-4c6c-82cb-513f44b0da1a
md"## Data Preparation"

# ╔═╡ 6faf9e94-e3e0-489b-9236-a5612894622f
Y, X_raw = unpack(drug_df, ==(:Drug), colname -> true)
...
# ╔═╡ 0569dce8-5cd0-40ab-a905-5187ff861010
piped_svm_cla_model1 = ContinuousEncoder() |> svm_cla_model1

# ╔═╡ 8f4085a6-52a6-41ea-b153-5c307e307fea
piped_svm_cla_mach1 = machine(piped_svm_cla_model1, X_fin, Y)

# ╔═╡ f74400b7-2f06-4d49-bbbd-14a3b4bb0c25
MLJ.fit!(piped_svm_cla_mach1, rows=train_set)

# ╔═╡ 8d7fada4-df5b-4d52-b4f2-9d884e312dde
Ŷ_t2 = MLJ.predict(piped_svm_cla_mach1, rows=test_set)

# ╔═╡ 32da9e36-fb06-4ae2-b483-b5106d9c3e20
cm2 = ConfusionMatrix()(Ŷ_t2, Y[test_set])

# ╔═╡ 1f4b0076-deb9-4533-b022-fad34ddf46bd
accuracy(cm2)

# ╔═╡ 906453c6-a91c-4310-9c50-0de6013c6bbb
svm_cla_model2 = MLJLIBSVMInterface.SVC(kernel=LIBSVM.Kernel.Polynomial)

# ╔═╡ 396274a5-c5f7-428a-b6a8-2d0c021c9521
piped_svm_cla_model2 = ContinuousEncoder() |> svm_cla_model2

# ╔═╡ 4b8a0afa-7bb2-4215-a283-a2070aec16b7
piped_svm_cla_mach2 = machine(piped_svm_cla_model2, X_fin, Y)

# ╔═╡ dc8afdaa-90a7-451b-b13b-8138460e261b
MLJ.fit!(piped_svm_cla_mach2, rows=train_set)

# ╔═╡ cedd9ce3-427d-4e6f-8bad-f1179328a5e0
Ŷ_t3 = MLJ.predict(piped_svm_cla_mach2, rows=test_set)

# ╔═╡ 649bdb20-edee-4b7d-b300-88a8b17ca819
cm3 = ConfusionMatrix()(Ŷ_t3, Y[test_set])

# ╔═╡ 0c912e71-a772-41a8-9efb-1c6eb0ce5841
accuracy(cm3)

# ╔═╡ 7fb02716-cdce-48a0-8a09-2cbbf7a9f651
md"### K Nearest Neighbor"

# ╔═╡ 2012d47d-06e2-49c1-a963-ff597b716f7a
KNNClassifier = @load KNNClassifier

# ╔═╡ ec6fdc16-3b0b-4a1d-abe7-8c147a78f93a
knn_model = ContinuousEncoder() |> KNNClassifier(K=3)

# ╔═╡ 7f9aa1e8-c715-497b-84b2-65c958f29f1c
knn_mach = machine(knn_model, X_fin, Y)

# ╔═╡ 61689983-fd61-4fa9-9f28-7584e093a742
MLJ.fit!(knn_mach, rows=train_set)

# ╔═╡ 9467794d-84f0-45f7-808f-f48caf063022
Ŷ_t4 = MLJ.predict(knn_mach, rows=test_set)

# ╔═╡ e862f153-2671-4728-ade0-ad4f4538c584
cm4 = confusion_matrix(mode.(Ŷ_t4), Y[test_set])

# ╔═╡ 8f7dce9a-36a3-4c8a-a52e-594ccd8cf23c
accuracy(cm4)

# ╔═╡ 193459f6-a47c-40bc-b153-3b8cad6c004d
md"### Boosted Trees"

# ╔═╡ 2c6f2dcf-754f-4cbf-af26-3141702ac353
Booster = @load EvoTreeClassifier pkg=EvoTrees

# ╔═╡ 4989ab71-7b49-4138-bafc-ceb834a2e17e
booster_model = Booster(tree_type="oblivious",rng=Xoshiro(456))

# ╔═╡ 94d92028-98bc-41d4-9523-ed883b2e4f5e
piped_booster_model = ContinuousEncoder() |> booster_model

# ╔═╡ 0728d143-2668-44e0-a6f9-a8e75f4c4102
piped_booster_mach = machine(piped_booster_model, X_fin, Y)

# ╔═╡ 4a799d98-b252-4a8e-b949-a224a8be4ff6
MLJ.fit!(piped_booster_mach, rows=train_set)

# ╔═╡ 390e62ec-40c0-4d28-96ac-a2238b8058bf
Ŷ_t5 = MLJ.predict(piped_booster_mach, rows=test_set)

# ╔═╡ 33be311f-86b3-4566-bc2e-24610225d355
cm5 = confusion_matrix(mode.(Ŷ_t5), Y[test_set])

# ╔═╡ 4a1e5c3b-8694-42a2-9937-f345e1bcd77c
accuracy(cm5)

# ╔═╡ d11d7ac3-c5e5-428e-a2cb-7dfae0343656
md"## Saving Models"

# ╔═╡ ebdad21c-bc79-4c44-bee2-bc9c8904761a
md"""We will save the two best models, namely the support vector machine with Gaussian kernel and the boosted tree"""

# ╔═╡ 550559bf-f2fc-451b-b719-60324932b3fd
# ╠═╡ disabled = true
#=╠═╡
MLJ.save(joinpath("./../saved/", "svm_gauss.jls"), piped_svm_cla_mach1)
  ╠═╡ =#

# ╔═╡ a2231f72-8273-419b-9d54-8c2c9ef2030a
# ╠═╡ disabled = true
#=╠═╡
MLJ.save(joinpath("./../saved/", "boosted_tree.jls"), piped_booster_mach)
  ╠═╡ =#

# ╔═╡ 9fbd2705-4b45-4eba-8f28-eac0c29348a8
