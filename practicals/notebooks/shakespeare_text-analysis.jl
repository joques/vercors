...
# ╔═╡ 3945afb0-2b2c-11f0-00e4-9f684507a43a
using PlutoUI

# ╔═╡ 68c96b5a-0ebc-4317-be8c-b0881b735937
using Flux

# ╔═╡ ff3e3052-50eb-4a07-90a9-9cb58657a40a
using Random

# ╔═╡ e521d04a-275c-4a0b-9fde-ff9b3c06922b
using CairoMakie

# ╔═╡ e32d59bd-00bd-48c6-a9e6-def4022736ba
using AlgebraOfGraphics

# ╔═╡ be3dd41a-373f-4929-970a-e161526d19c9
using StatsBase

# ╔═╡ 124af0a5-679b-4e89-8059-146b12d32a99
PlutoUI.TableOfContents()

# ╔═╡ 92230f31-8646-4dad-870f-8cf60467cdc7
md"""This notebook presents an application of recurrent neural networks, namely long short-term memory (LSTM) models to predict an outcome. Here, we read a sequence of texts and predict the next character."""

# ╔═╡ 95c2beb9-a58c-4bf7-a0b1-34b85f918305
md"# Long Short Term Memory (LSTM) with Flux"

# ╔═╡ eb2bbeab-0ce2-41a2-9900-4d5d49e31ccc
md"## Configuration"

# ╔═╡ 8f576241-d92f-4387-82f2-e32871956e68
...
# ╔═╡ d463a429-926c-478d-bfac-8815972e2e75
md_args = ModelArgs()

# ╔═╡ 40353f8e-d04d-47ea-9e36-b257aa805405
md"## Load and Prepare Data"

# ╔═╡ ff710afc-80bc-4b4d-9d7f-53bf0eb578ec
cori_txt = String(read("../dataset/coriolanus.txt"))

# ╔═╡ 25dbc254-a85e-4a19-b6ed-a0107651e361
the_alphabet = [unique(cori_txt)..., '_']

# ╔═╡ 70727897-3ce0-4ffb-8b5b-1c3a5e6bd619
stop_char = '_'

# ╔═╡ d9f8e5bc-76fc-4757-bf3f-4ec4337db801
alphabet_size = length(the_alphabet)

# ╔═╡ 2ec6632d-1bc4-403b-868d-84f1106da899
Xtext = [collect(t) for t in Flux.chunk(cori_txt, md_args.batch_size)]

# ╔═╡ af69c511-a452-454e-80d2-6cfad5013932
Ytext = [collect(t) for t in Flux.chunk(cori_txt[2:end], md_args.batch_size)]

# ╔═╡ 3bbce459-640b-4553-b346-1f3ad52dfb27
 Xs = Base.Iterators.partition(Flux.batchseq(Xtext, stop_char), md_args.seq_len)

# ╔═╡ 3c465d78-8f2b-46d7-832b-8b2c0cfc575e
Ys = Base.Iterators.partition(Flux.batchseq(Ytext, stop_char), md_args.seq_len)

# ╔═╡ c7bcb1c0-a72f-474c-95e6-c439b09deb7b
Xs1 = [Flux.onehotbatch.(bs, (the_alphabet,)) for bs in Xs]

# ╔═╡ 15d15b2b-c6d2-4ae5-b703-53d618d4f842
Ys1 = [Flux.onehotbatch.(bs, (the_alphabet,)) for bs in Ys]

# ╔═╡ 370e0e92-5a32-4a1e-aa48-adb7261f28c1
L = length(Xs1)

# ╔═╡ 39dfb2a9-f59b-41eb-88c8-4efb4f4e4d8c
perm_idx = Random.shuffle(1:length(Xs1))

# ╔═╡ 692b3d9f-b7f8-47f1-96ae-829371bbc7f3
train_lim = floor(Int, (md_args.train_split) * length(Xs1))

# ╔═╡ e5eb1f11-d1f4-4428-92a9-8e41b9fa08e1
X_train = Xs1[perm_idx[1:train_lim]]

# ╔═╡ f3680376-2b8a-4368-a0a4-93fea92fb635
Y_train = Ys1[perm_idx[1:train_lim]]

# ╔═╡ a227ee46-636c-4275-98c7-554240c3ceb9
X_test = Xs1[perm_idx[(train_lim+1):end]]

# ╔═╡ 059c0f62-3513-4301-a483-a203a01471a3
Y_test = Ys1[perm_idx[(train_lim+1):end]]

# ╔═╡ 1d7af4d4-1766-418e-9618-ffbd661c0919
md"## Build Model"

# ╔═╡ 8e6405c2-7fe0-49c6-b723-fd50733b5dc1
txt_an_model = Chain(LSTM(alphabet_size => 128),LSTM(128 => 128), Dense(128 => alphabet_size))

# ╔═╡ 210aab4d-4cfa-48ea-8147-fbc461ef98e3
function my_loss(m, X, Y)
	Flux.reset!(m)
	return sum(Flux.logitcrossentropy.([m(x) for x in X], Y))
end

# ╔═╡ 9e4cdca1-e601-4d40-a88f-d5e5314fa93c
the_opt = Flux.setup(Adam(md_args.learning_rate), txt_an_model)

# ╔═╡ 778cf5be-2c92-4aac-8335-cf6065b8978e
md"## Train Model and Evaluate"

# ╔═╡ 4b458565-4a4a-4cdf-9d81-2fa12e1d8611
begin
	training_loss = Float64[]
    epochs = Int64[]

	for epoch in 1:md_args.epochs
		@info "Training, epoch $(epoch) / $(md_args.epochs)"
		Flux.train!(my_loss, txt_an_model, zip(X_train, Y_train),the_opt)
        push!(epochs, epoch)
        push!(training_loss, (Float64)(sum(my_loss.(Ref(txt_an_model), X_train, Y_train)/(md_args.batch_size * md_args.seq_len * length(X_train)))))
    end
end

# ╔═╡ b87c6add-c53d-4970-b582-b3d6b334e8fa
mapping(epochs, training_loss) * visual(Lines) |> draw

# ╔═╡ 22d62533-084f-4326-90c8-88f210998d90
(Float64)(sum(my_loss.(Ref(txt_an_model), X_test, Y_test)/(md_args.batch_size * md_args.seq_len * length(X_test))))

# ╔═╡ cfa2200a-3577-4ebb-ae30-e47a1d901953
md"## Usage"

# ╔═╡ 36ac5024-2643-4d59-8cd1-73637eb38260
a_seed = string(rand(the_alphabet))

# ╔═╡ e56e4d70-2ce4-4ba3-8f99-c14a0bb6cf40
a_buf = IOBuffer()

# ╔═╡ acd522ca-2bec-40d1-8dab-ba9dcb7b58c1
write(a_buf, a_seed)

# ╔═╡ 2c5a91a0-ac85-4865-b4e1-061db7ecf551
for i = 1:200
	pred_sa1 = [Flux.onehot(a_char, the_alphabet) for a_char in collect(a_seed)][end]
	seg = wsample(the_alphabet, reshape(softmax(txt_an_model(reshape(pred_sa1, length(pred_sa1), 1))), 68))
	write(a_buf, seg)		
end

# ╔═╡ c6458b08-ef39-48d8-9627-2371bd34ed73
println(String(take!(a_buf)))
...
manifest_format = "2.0"
project_hash = "2852bdfa5b699c2ed827b36c2f318eae3578baa3"
