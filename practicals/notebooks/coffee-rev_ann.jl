...
# ╔═╡ 44bc448a-204b-11f0-22f5-4f16ae174fa3
using PlutoUI

# ╔═╡ fbb02845-2e78-4948-8460-fdf6f3cc1f22
using CSV

# ╔═╡ be6784f1-d161-43e1-8371-3a81f5541c84
using DataFrames

# ╔═╡ 68b99094-c7ac-4936-a830-9ceeb8177d51
using Flux

# ╔═╡ eec1e444-8f0e-4a52-8120-afd25d6634d4
using MLJ

# ╔═╡ c2435ffd-c5dd-452e-bbd7-46cdc639f8fe
using MLJFlux

# ╔═╡ 9b9d0c94-9861-4fdd-9287-1b0ada91aa81
using Statistics

# ╔═╡ 20baba7f-aae8-4603-8bf5-82389ee2311e
using CairoMakie

# ╔═╡ 61d780b9-009b-45af-b5e1-c8f1b96cd10d
using AlgebraOfGraphics

# ╔═╡ 8c28cf07-cbbd-4f8b-847d-51e75c5da163
using Random

# ╔═╡ 47c97b2f-a2d4-4dce-8d45-2f115a555f69
PlutoUI.TableOfContents()

# ╔═╡ 7c6ee07d-0aef-4fa0-bbfd-f2d5a32c4475
md"# Coffee Revenue Prediction -- ANN version"

# ╔═╡ 58b044a4-3afb-405e-9a3c-8c64377a12ee
md"## Data Loading"

# ╔═╡ 4c77dc26-37b9-49f0-9d46-abcdf34444cd
coffee_rev_df = DataFrame(CSV.File("../dataset/coffee_shop_revenue.csv", header=1))

# ╔═╡ e54e5383-6540-443f-89d8-01b05e18796e
fs_coffee_df = select(coffee_rev_df, [1, 2, 5, 7])

# ╔═╡ 861d0771-f332-4b13-88c0-3810317957f6
fs_coffee_df_train = fs_coffee_df[1:1600, :]

# ╔═╡ 9c2551b9-dfd9-43ad-8ffc-c07716e703ae
fs_coffee_df_test = fs_coffee_df[1601:2000, :]

# ╔═╡ 70b98806-8111-4bc9-8ba6-f8d8e58c883c
X_train = Float32.(Matrix(select(fs_coffee_df_train, Not([:Daily_Revenue]))))

# ╔═╡ 3f3cf3ae-d329-4146-a130-5ba4a80cb357
X_train_st = Flux.normalise(X_train)

# ╔═╡ f983b817-2f3f-4452-8958-db8615ff17e3
Y_train = Float32.(fs_coffee_df_train.Daily_Revenue)

# ╔═╡ 9b8c480a-1a36-4ee3-934e-14c867745727
X_test = Float32.(Matrix(select(fs_coffee_df_test, Not([:Daily_Revenue]))))

# ╔═╡ dfdb18a4-922a-4277-8ed9-36cfa3c9b559
X_test_st = Flux.normalise(X_test)

# ╔═╡ 1df225ef-6094-4194-be4b-6eb0abb425fb
Y_test = Float32.(fs_coffee_df_test.Daily_Revenue)

# ╔═╡ 607024ae-6675-496a-8e3a-d3563257784b
train_loader = Flux.DataLoader((X_train_st', Y_train'), batchsize=32, shuffle=true)

# ╔═╡ c06ac1e5-1886-4dd2-a59b-64fef4a6540d
md"## Build Model"

# ╔═╡ 97d271ab-838d-4a00-9a62-3d5798193380
cof_rev_model = Chain(Dense(3, 5, relu), Dense(5,2), Dense(2,1))

# ╔═╡ 75331d09-7831-43ea-956d-d53a0fe3d1ef
my_loss(m, x, y) = Flux.mse(m(x), y) |> sqrt

# ╔═╡ dda3eb56-0726-45f9-8e0a-280535f97157
the_opt = Flux.setup(Adam(), cof_rev_model)

# ╔═╡ ce0e770c-da52-42bd-ba49-c874353c25f4
begin
	training_loss = Float64[]
    epochs = Int64[]

    for epoch in 1:300
        Flux.train!(my_loss, cof_rev_model, train_loader, the_opt)
        push!(epochs, epoch)
        push!(training_loss, my_loss(cof_rev_model, train_loader.data[1],train_loader.data[2]))
    end
end

# ╔═╡ 22c53117-af7d-4e3f-91bb-16303a5f374a
mapping(epochs, training_loss) * visual(Lines) |> draw

# ╔═╡ 59c4f323-7c0f-4984-80ec-71442bb8d5ff
Flux.mse(cof_rev_model(X_train_st'), Y_train') |> sqrt

# ╔═╡ 20c8d78b-233e-49e8-8920-6e96689b314e
Flux.mse(cof_rev_model(X_test_st'), Y_test') |> sqrt

# ╔═╡ 1d5ae2d1-f157-4b4e-bf59-1e427f500227
md"## Model with MLJ Interface"

# ╔═╡ f942dfc7-6f89-46eb-9587-c25d7aa774c0
NeuralNetworkRegressor = @load NeuralNetworkRegressor pkg=MLJFlux

# ╔═╡ 5b649fe3-673e-474d-b8e9-d272d68826bb
fs_coffee_df_cpy = deepcopy(fs_coffee_df)

# ╔═╡ ec6f53a7-6e78-413d-ac4c-2afa2a664caf
schema(fs_coffee_df_cpy) |> DataFrames.DataFrame

# ╔═╡ ddcdf096-eba7-4bef-a107-4af9be7f9910
norm_mach = machine(Standardizer(), fs_coffee_df_cpy)

# ╔═╡ 2c315a9a-c8b7-43fe-8aee-f8e2208ccb88
MLJ.fit!(norm_mach)

# ╔═╡ 86e2aa27-a464-466d-8185-7c89aac6ea0e
st_data = MLJ.transform(norm_mach, fs_coffee_df_cpy)

# ╔═╡ 90128c45-954d-464b-8c84-1ef7f978a9ae
Y_cf_st, X_cf_st = MLJ.unpack(st_data, ==(Symbol("Daily_Revenue")), colname -> true)

# ╔═╡ 19f6660a-9774-42cd-b9b7-926db9bdd4a2
train_set2, test_set2 = MLJ.partition(eachindex(Y_cf_st), 0.80)

# ╔═╡ a7166bea-09a1-4873-aa6e-b7df9f92765b
cf_builder = MLJFlux.@builder begin
    Chain(
        Dense(3, 5, relu),
        Dense(5, 2, relu),
        Dense(2, 1),
...
# ╔═╡ 00ffca82-2312-4af2-b964-4562c68cca06
cof_rev_model2 = NeuralNetworkRegressor(
    builder=cf_builder,
    rng=123,
    epochs=300
)

# ╔═╡ 456cb0a4-c001-41f9-8d58-a29925e70eac
cof_rev_mach = machine(cof_rev_model2, X_cf_st, Y_cf_st)

# ╔═╡ 21d97c69-b543-45e9-9e1b-b273bc305fa0
MLJ.fit!(cof_rev_mach, rows=train_set2, verbosity=2)

# ╔═╡ 680985cf-d65c-420f-b424-f92121ab2060
mapping(1:301, report(cof_rev_mach).training_losses) * visual(Lines) |> draw

# ╔═╡ 02a593dd-f896-4eea-850e-4f5cd1782731
evaluate!(cof_rev_mach, resampling=CV(nfolds=5), measure=rmse)

# ╔═╡ 66e83479-f86a-425d-b588-d4b44fd4add7
rmse(MLJ.predict(cof_rev_mach, X_cf_st[test_set2,:]), Y_cf_st[test_set2])

# ╔═╡ 537f24b6-faab-4580-a663-71d0608bd3d4
evaluate!(cof_rev_mach, resampling=CV(nfolds=5), measure=l2)

# ╔═╡ 7fef2f18-8ff4-4cc0-b1d7-4480da44e8b7
l2(MLJ.predict(cof_rev_mach, X_cf_st[test_set2,:]), Y_cf_st[test_set2])

# ╔═╡ 0b7d8c43-f1e8-43a0-a6c5-77c3042d1df1
lrr = range(cof_rev_model2, :lambda, lower=0.01, upper=0.1)

# ╔═╡ 10891306-9580-4b3b-a5fa-4620eb9a77ec
l_epc = range(cof_rev_model2, :epochs, lower=10, upper=400)

# ╔═╡ 86da0a74-5c52-4696-a6c6-afc3bb9c5581
tuned_ann_coffee_model = TunedModel(model=cof_rev_model2, ranges=[lrr, l_epc], measure=rmse)

# ╔═╡ 609d305f-216c-4d4b-b930-ccf0fbd11fe8
cof_rev_mach2 = machine(tuned_ann_coffee_model, X_cf_st, Y_cf_st)

# ╔═╡ 6f92875b-9086-4279-806c-a6d37dd605d4
MLJ.fit!(cof_rev_mach2, rows=train_set2)

# ╔═╡ 455a0bfe-8282-42ad-99c1-5db5fd995773
rmse(MLJ.predict(cof_rev_mach2, X_cf_st[test_set2,:]), Y_cf_st[test_set2])

# ╔═╡ 3c83abda-cdd7-4a36-81a9-9c97ccad36fc
l2(MLJ.predict(cof_rev_mach2, X_cf_st[test_set2,:]), Y_cf_st[test_set2])

# ╔═╡ ab657a21-7b0f-41d2-b255-e661988c10cc
report(cof_rev_mach2)

# ╔═╡ 5dbefc00-9669-48eb-9158-1db1031e7179
mapping(1:363, report(cof_rev_mach2).best_report.training_losses) * visual(Lines) |> draw
...
manifest_format = "2.0"
project_hash = "39e531105abb1fc370dcd8a6cfa1d2be1cc83f5b"
