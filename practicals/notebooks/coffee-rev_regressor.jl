...
# ╔═╡ 19dccba2-04aa-11f0-3829-f545992ef136
using PlutoUI

# ╔═╡ 0398838d-ec94-4982-8088-f6927610f248
using CSV

# ╔═╡ 9879504b-7258-4115-bf69-dbc41fd6420c
using DataFrames

# ╔═╡ ea09152b-281c-4604-b4bf-46f36bc9d42e
using CairoMakie

# ╔═╡ 3e6fef1c-f91c-4337-967e-0f90c3d0e488
using AlgebraOfGraphics

# ╔═╡ b44bfba4-352e-4cb6-8919-93157c508284
using MLJ
...
# ╔═╡ ee9f72fa-eacd-44a6-90f5-781d0e765b19
using MLJDecisionTreeInterface

# ╔═╡ b5366629-8a1a-476f-a475-8defcddd9e75
using BetaML
...
md"Coffee Revenue Prediction - Part II"

# ╔═╡ 318a5811-10f0-4545-abfb-5a87e473f622
md"""This notebook contains a prediction model for coffee revenue. It follows the previous linear models developed to predict coffee revenue. Here we use a Support Vector Regressor"""

# ╔═╡ cf2ca46a-aeec-4946-9c1a-89ceaf994b13
md"## Data Loading"

# ╔═╡ 1655d8a1-23e6-4503-9caa-43f4617fe7bc
coffee_rev_df = DataFrame(CSV.File("../dataset/coffee_shop_revenue.csv", header=1))

# ╔═╡ c534503f-61bf-4b27-9545-d2b865b97887
fs_coffee_df = select(coffee_rev_df, [1, 2, 5, 7])

# ╔═╡ af7b4312-8da4-4fe9-91b8-bbb1ea6a9eb5
coffee_df1 = coerce(fs_coffee_df, autotype(fs_coffee_df, :discrete_to_continuous))

# ╔═╡ 3b2d2b15-623c-4b0a-922e-632b96983e33
schema(coffee_df1) |> DataFrames.DataFrame

# ╔═╡ 86ba37ac-c322-4b88-966e-02f0e976afc3
Y, X = MLJ.unpack(coffee_df1, ==(Symbol("Daily_Revenue")), colname -> true)

# ╔═╡ 9d10cd26-f926-4b3a-88fb-b8dca58cfee6
X_fs_mach = machine(Standardizer(), X)

# ╔═╡ 9d65befc-a16b-41c6-a89b-555d9d2fa561
X_norm = MLJ.transform(MLJ.fit!(X_fs_mach), X)

# ╔═╡ 71d30ed0-6331-417a-aaff-34df4b84fac5
Y_fs_mach = machine(Standardizer(), Y)

# ╔═╡ cdcd0130-cd3d-4105-87c4-7f1183267c0d
Y_norm = MLJ.transform(MLJ.fit!(Y_fs_mach), Y)

# ╔═╡ 520a20e3-4f35-428c-992b-9f1cbcc9739e
train_set, test_set = MLJ.partition(eachindex(Y_norm), 0.80)

# ╔═╡ 30c30ce8-6024-4f81-bd87-6fba5e0e2ec3
md"## Models"

# ╔═╡ 7b4793db-77a5-45ca-9841-ae0d1e11e29f
md"### Plain Linear Model"

# ╔═╡ ff2efa59-c863-4b4f-a5d6-cffd50d0068b
LReg = @load LinearRegressor pkg = "MLJLinearModels" verbosity=0

# ╔═╡ 8d325538-f9e2-40c8-81fb-bcf56e134820
reg_model = LReg(solver=Analytical())

# ╔═╡ 54ab1fc5-7856-496b-a604-d6e860188aa1
reg_mach = machine(reg_model, X_norm, Y_norm)

# ╔═╡ 9ce0787a-5c65-4e58-b075-ba9d8c14f703
MLJ.fit!(reg_mach, rows=train_set)

# ╔═╡ ad60d059-027c-4eb5-9dca-f83e0bd14610
evaluate!(reg_mach, resampling=CV(nfolds=10,shuffle=true,rng=42), measures=[rms], repeats=3)

# ╔═╡ b5972fc8-aa5c-45c3-8282-6dc3a0b7a813
rmse(MLJ.predict(reg_mach, X_norm[train_set,:]), Y_norm[train_set])

# ╔═╡ 083c6112-ec53-494a-87fd-32516c4f0752
rmse(MLJ.predict(reg_mach, X_norm[test_set,:]), Y_norm[test_set])

# ╔═╡ 149f9cd7-b859-41c3-ac10-b296039dbeb2
fp = fitted_params(reg_mach)

# ╔═╡ f8eab0d1-7a69-4ef5-988a-8b24a8a8664a
for (col_name, weight) in fp.coefs
    println("$(rpad(col_name, 8)):  $(round(weight, sigdigits=3))")
end

# ╔═╡ 1b622615-10a8-4280-9934-b1bbd9128b2b
println("Intercept: $(round(fp.intercept, sigdigits=3))")

# ╔═╡ 3303f03f-b6f1-40d3-9ce8-11da19202df2
md"### Support Vector Regressor"

# ╔═╡ c65b632b-e5a3-41c1-9f20-e01912980c57
EpsilonSVR = @load EpsilonSVR pkg=LIBSVM

# ╔═╡ 32f0abe5-a216-41d9-8026-b96d06811047
svr_model = EpsilonSVR(kernel=LIBSVM.Kernel.Linear)

# ╔═╡ 3ca62426-0108-4d5e-a558-184ec0ef8a7c
svr_mach = machine(svr_model, X_norm, Y_norm)

# ╔═╡ 59188f92-f841-4513-8fa4-ccaef6ec7eef
MLJ.fit!(svr_mach, rows=train_set)

# ╔═╡ 351561cf-eb65-439f-ab77-5e635536d0ed
rmse(MLJ.predict(svr_mach, X_norm[train_set,:]), Y_norm[train_set])

# ╔═╡ 5d6fee21-d7ee-4c82-b73c-2ca928e9405e
rmse(MLJ.predict(svr_mach, X_norm[test_set,:]), Y_norm[test_set])

# ╔═╡ 885e64e9-0474-4eba-b5e0-7e53510cbab0
svr_model2 = EpsilonSVR(kernel=LIBSVM.Kernel.Polynomial)

# ╔═╡ 4fe98e6b-07cc-4937-987b-a0e865594072
svr_mach2 = machine(svr_model2, X_norm, Y_norm)

# ╔═╡ 5cf9f152-d196-4023-9331-36c8b20df30f
MLJ.fit!(svr_mach2, rows=train_set)

# ╔═╡ 2234b417-b07b-4d16-a333-ab72b480cb49
rmse(MLJ.predict(svr_mach2, X_norm[train_set,:]), Y_norm[train_set])

# ╔═╡ 53cb907d-8ac4-42d9-9b54-a403518605c6
rmse(MLJ.predict(svr_mach2, X_norm[test_set,:]), Y_norm[test_set])

# ╔═╡ f6e4f802-0ca9-4696-965f-8f0a570085f2
svr_model3 = EpsilonSVR(kernel=LIBSVM.Kernel.RadialBasis)

# ╔═╡ 6fe65f37-ce6e-47ad-9ccc-f889f629bb83
svr_mach3 = machine(svr_model3, X_norm, Y_norm)

# ╔═╡ 578b20c8-8d7f-493a-a502-40ff2ad96980
MLJ.fit!(svr_mach3, rows=train_set)

# ╔═╡ 0c2e1974-9ddf-4f67-a9b1-fa62951be1f8
rmse(MLJ.predict(svr_mach3, X_norm[train_set,:]), Y_norm[train_set])

# ╔═╡ 6bda7efa-236d-4967-b67e-db3d1a20490c
rmse(MLJ.predict(svr_mach3, X_norm[test_set,:]), Y_norm[test_set])

# ╔═╡ c6fd17c8-2d91-49e6-ae1c-05d1130d4bc4
svr_model4 = EpsilonSVR(kernel=LIBSVM.Kernel.Sigmoid)

# ╔═╡ 0fe1f6a7-2d01-4fd3-9c2d-e2561886f078
svr_mach4 = machine(svr_model4, X_norm, Y_norm)

# ╔═╡ c20fb242-f90e-437a-9c17-8b5d88a0bdc9
MLJ.fit!(svr_mach4, rows=train_set)

# ╔═╡ 83cc1337-0abb-4005-8df0-ad01071d4543
rmse(MLJ.predict(svr_mach4, X_norm[train_set,:]), Y_norm[train_set])

# ╔═╡ 5d8f8b3b-236d-44e4-b4cd-5b90b2429616
rmse(MLJ.predict(svr_mach4, X_norm[test_set,:]), Y_norm[test_set])

# ╔═╡ d2ed8c90-8163-455c-9276-2d0311716337
md"""**Note**: So far the Gaussian kernel yields the best regression model."""

# ╔═╡ e7cddf53-4eef-42b6-87de-ba90f8d7a4a0
md"### Decision Tree Regressor"

# ╔═╡ b6311152-0143-4bfc-8d7b-54cca3727240
DecisionTreeRegressor = @load DecisionTreeRegressor pkg=DecisionTree

# ╔═╡ dceed24e-754d-47e7-bc99-7b10f806ba87
dt_model = DecisionTreeRegressor(max_depth=5, min_samples_split=3)

# ╔═╡ 41d77189-6703-4546-aed3-7d6a141cc091
dt_mach = machine(dt_model, X_norm, Y_norm)

# ╔═╡ ec1b3082-99b6-4211-9b89-c0d348266ec3
MLJ.fit!(dt_mach)

# ╔═╡ 2fd5593e-2e2e-440d-9f58-e9e67989ecfb
rmse(MLJ.predict(dt_mach, X_norm[train_set,:]), Y_norm[train_set])

# ╔═╡ eff34771-0235-4b08-9477-9ef010563d59
rmse(MLJ.predict(dt_mach, X_norm[test_set,:]), Y_norm[test_set])

# ╔═╡ 71a92177-8487-42e5-b791-ef368104d799
fitted_params(dt_mach).tree

# ╔═╡ 5b74bd27-7b0c-464e-be0f-28950b25bfcb
feature_importances(dt_mach)

# ╔═╡ fe3e5323-f2d3-46fe-b0ea-652508ce97f7
RandomForestRegressor = @load RandomForestRegressor pkg = "BetaML" verbosity=0

# ╔═╡ f6811edc-4725-4848-8172-1dc2b5b2bb95
dt_model2 = RandomForestRegressor(max_depth=5)

# ╔═╡ b1d4da30-7e50-4292-b53c-31aff6a6d30b
dt_mach2 = machine(dt_model2, X_norm, Y_norm)

# ╔═╡ 435d2793-9c03-4083-a54b-56ab17150d7f
MLJ.fit!(dt_mach2)

# ╔═╡ f17946f1-58ee-458b-adf6-cbbe2754d8f9
rmse(MLJ.predict(dt_mach2, X_norm[train_set,:]), Y_norm[train_set])

# ╔═╡ b0a5a349-d0cb-4499-91db-d08c5685f163
rmse(MLJ.predict(dt_mach2, X_norm[test_set,:]), Y_norm[test_set])

# ╔═╡ 8c205306-bacf-433b-8188-f4d9ac8ddb50
fitted_params(dt_mach2)
...
manifest_format = "2.0"
project_hash = "3b05689ef11430387b0baad9d7c52121d3d1dff0"
...
git-tree-sha1 = "56b79843ef4f7ebbb72d28e88e83251c033b1613"
