...
# ╔═╡ b612bfcc-ff50-11ef-3aeb-11fbbce7047d
using PlutoUI

# ╔═╡ 4762b72b-a4af-4a22-b69d-d02b81046dee
using CSV

# ╔═╡ c1631b09-5b75-432e-b3bf-ddec8ab74db3
using DataFrames

# ╔═╡ c3e7932e-f14c-4d90-8b46-21bc5646dd07
using MLJ

# ╔═╡ f38523aa-c3df-4708-bb53-328db1d1f749
using Statistics

# ╔═╡ 8e99e6dd-8d4f-4fc1-ae53-289fbe1f02b1
using StatsBase

# ╔═╡ 47368853-3e66-47a5-91ec-e9714135a5d6
using CairoMakie

# ╔═╡ 7d507ed1-d7f9-417d-af66-b598de8fc9ee
using AlgebraOfGraphics

# ╔═╡ 34935de3-460a-4217-9079-5073204b0d93
using Random

# ╔═╡ 0c2cd5d1-4e27-4188-bf06-22745333f503
using MLJLinearModels

# ╔═╡ 594a19e3-5e2e-4369-8d88-6c05713649c1
using EvoTrees

# ╔═╡ 9ea9b917-7647-4ba3-b459-fe3661f8d36f
PlutoUI.TableOfContents()

# ╔═╡ e0e537e1-dc5a-48b3-97b1-0a751d808335
md"# Logistic Regression in Julia"

# ╔═╡ f68aaa57-56ea-4c1b-ba70-31c8d5612b7f
md"""In this notebook, we build different types of classifiers. The algorithms we consider include logistic regression and a tree-based approach. We use a social network ads dataset accessible [here](https://www.kaggle.com/datasets/dragonheir/logistic-regression). A copy of the dataset is available in the dataset folder. You can follow the link to get the metadata on the data."""

# ╔═╡ 6982f2b1-a366-4c5c-ba65-5e75e66750cd
md"## Data Exploration"

# ╔═╡ bacef7f2-a675-4e0f-a58e-c028835f3c81
ads_df = DataFrame(CSV.File("../dataset/social_netowrk_ads.csv", header=1))

# ╔═╡ 9c4ce9cf-c136-4b47-bf5b-18492a04b8cd
for col_name in names(ads_df)
        println(col_name)
end

# ╔═╡ 75eb36b4-8b9c-4e9c-aab0-eb58fef856a8
ads_df1 = select(ads_df, [2,3,4,5])

# ╔═╡ 9772c645-b5b2-4625-9441-49b98624de90
data(ads_df1) *
    visual(Violin) *
    mapping(:Gender, :Purchased => "Purchased", color=:Gender) |> draw()

# ╔═╡ 55fe1756-4564-4237-b03d-6de550acc671
layers = histogram(normalization=:pdf) + AlgebraOfGraphics.density()

# ╔═╡ f5f7803c-0b4a-4e62-bfe2-6bcb98b1b015
schema(ads_df1)

# ╔═╡ 02a6d079-1aed-472e-9687-aeeaac76052b
describe(ads_df1)

# ╔═╡ 407c2e22-2c6a-4344-b07d-fc6241ab6bfa
levels(ads_df1.Gender)

# ╔═╡ 59365bbc-994b-4b8c-a582-18ccded6aef2
levels(ads_df1.Age)

# ╔═╡ 604e7baa-a5fc-40d4-ba07-c4aa1db71093
levels(ads_df1.EstimatedSalary)

# ╔═╡ 54293014-c42d-47da-b974-72c27d5676fa
levels(ads_df1.Purchased)

# ╔═╡ a8790ae3-2a73-44cc-af37-a8598ed216b6
coerce!(ads_df1, :Age => Continuous, :EstimatedSalary => Continuous)

# ╔═╡ 54d77264-8cf2-4f07-9583-154b2d49edb6
coerce!(ads_df1, Textual => Multiclass)

# ╔═╡ 28749c3c-07ba-49cf-a853-df7154498d76
coerce!(ads_df1, :Purchased => OrderedFactor)

# ╔═╡ 2cf13803-17dd-4eb1-95e2-a3e69cf38860
y, X = unpack(ads_df1, ==(:Purchased))

# ╔═╡ 27afcaa5-00bd-41dc-9deb-48ee7855b13f
gender_mach = machine(OneHotEncoder(), X) |> fit!

# ╔═╡ 80df7134-e1be-4011-8c42-7c2bba92f7f3
ads_feature_df = MLJ.transform(gender_mach, X)

# ╔═╡ 265d7421-c4b2-45f9-af99-d989d97d7cb0
schema(ads_feature_df) |> DataFrames.DataFrame

# ╔═╡ fe6841c0-329c-44a6-a804-61a7b760cce3
md"## Prepare Dataset"

# ╔═╡ d89c0018-977e-402c-b257-2d2edb306a1b
train_set, test_set = partition(eachindex(y), 0.85; rng=Xoshiro(456))

# ╔═╡ 0f678da8-7aed-4841-b222-365705b2f78b
md"## Building Models"

# ╔═╡ 1920800d-f08a-461c-8f43-d88eb5ca80c7
md"### Logistic Classifier"

# ╔═╡ 131ce946-eb04-45a7-afe6-b96f2e9c0d60
LogClassifier = @load LogisticClassifier pkg=MLJLinearModels

# ╔═╡ 13dfcebd-fdfe-4542-aba8-c104ac6d7f4c
log_cla = LogClassifier(lambda=0.02, fit_intercept=false)

# ╔═╡ 360d1ee7-7c28-4004-a8ee-f4591bf38d83
log_cla_mach = machine(log_cla, ads_feature_df, y)

# ╔═╡ e9192cb5-149b-4a0d-a7ba-98af803f17c8
fit!(log_cla_mach, rows=train_set)

# ╔═╡ 30b8c272-57b1-4b6a-9eda-f6c6a851e570
ŷ_test1 = MLJ.predict(log_cla_mach, rows=test_set)

# ╔═╡ 79eb48c7-62f2-47e1-9f14-ca1eefd632fa
confmat(mode.(ŷ_test1), y[test_set])

# ╔═╡ b6316895-7f83-4674-9197-e27c7f5c1c88
md"### Tree-based Classifier"

# ╔═╡ f2ca0bc0-5ee2-44ff-8904-9e7487375c1e
Booster = @load EvoTreeClassifier pkg=EvoTrees

# ╔═╡ d0b59d14-24da-467c-9a3a-87a57a4492c8
booster = Booster(tree_type="oblivious",rng=Xoshiro(456))

# ╔═╡ 78bc56bf-a261-48bc-8d4e-91cbf0a5ec6d
pipe_model = ContinuousEncoder() |> booster

# ╔═╡ 323d2a30-1f7a-4ade-9733-abd94a0f622b
pipe_mach = machine(pipe_model, X, y)

# ╔═╡ 3e296d59-a0a1-4932-88e1-26e688029565
fit!(pipe_mach, rows=train_set)

# ╔═╡ d8048952-7cf1-4b57-84e3-55f8628c68af
ŷ_test = MLJ.predict(pipe_mach, rows=test_set)

# ╔═╡ efba3022-ab4b-402a-8803-63ba2183c411
confmat(mode.(ŷ_test), y[test_set])

# ╔═╡ 5734e995-9525-488a-b5c7-3f842eae5274
(40+14)/(40+14+6)

# ╔═╡ 9c02392e-2b9e-4920-90f1-0271d396cce3
...
manifest_format = "2.0"
project_hash = "a140a8964fb0ef0a2f66c0b9c6d9771b0a4d7b2d"
