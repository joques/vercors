...
# ╔═╡ 27c302fe-251b-11f0-2889-b95767ed151e
using PlutoUI

# ╔═╡ 118c8fb8-084d-49eb-8bb1-eea25c60e94d
using CSV

# ╔═╡ a3e98431-a3f8-4920-a4ba-3aa64a0d7ca1
using DataFrames

# ╔═╡ 10e0f6ed-b25a-40b9-a2ac-cfde540570cd
using Flux

# ╔═╡ 5c065d1f-51d5-4ae1-934d-233688f21f51
using MLJFlux

# ╔═╡ 451334c3-fbae-46c2-85ae-ec04be702ce5
using Statistics

# ╔═╡ 18c2743a-bad4-439a-a5d5-e95bcc9c395d
using Random

# ╔═╡ fc55f16b-3d74-4702-b176-19316ba33527
using CairoMakie

# ╔═╡ cf2cd1d1-904a-4dfd-9484-b5a066d574ed
using AlgebraOfGraphics

# ╔═╡ 7dc5c736-0885-4410-85ec-c6e71d7ecd64
using MLJ

# ╔═╡ befcc1cc-6e5f-4cfa-a0bf-e8a95c09ea23
using CategoricalDistributions

# ╔═╡ 7bae6ec1-c5ca-422e-8778-788786fbb482
PlutoUI.TableOfContents()

# ╔═╡ 37b276d9-8ef1-4399-b3dc-91962c5f9740
md"# Social Neotwork Classifier -- ANN Version"

# ╔═╡ 0f279ba8-a85f-4847-a5f0-f33846726cdf
md"## Loading Data"

# ╔═╡ 5466509f-6216-431d-ba6e-2c52aac5e0c8
ads_df = DataFrame(CSV.File("../dataset/social_netowrk_ads.csv", header=1))

# ╔═╡ 554b57e8-06e7-418e-ab42-ce83ecf6f70d
ads_df1 = select(ads_df, [2,3,4,5])

# ╔═╡ ff4867b2-8647-4968-b8c3-fcde30288c98
coerce!(ads_df1, :Age => Continuous, :EstimatedSalary => Continuous)

# ╔═╡ 40e5e77f-e8bc-46a3-b4a8-ae924bcd76dd
coerce!(ads_df1, Textual => Multiclass)

# ╔═╡ 28fa6b06-b832-4552-a3be-51ccabb3621d
Y, X = unpack(ads_df1, ==(:Purchased))

# ╔═╡ becb1de9-5239-4197-a528-7ca70d8cd8b9
stan_mach = machine(Standardizer(), X) |> MLJ.fit!

# ╔═╡ b966bf8b-917f-4e22-9c76-b85d1efc8639
X_st = MLJ.transform(stan_mach, X)

# ╔═╡ 7a466b7f-b4be-4df8-bc77-353b60701288
gender_mach = machine(OneHotEncoder(), X_st) |> MLJ.fit!

# ╔═╡ 640b7b14-fe59-4081-b9aa-719269ea432a
X_trans = MLJ.transform(gender_mach, X_st)

# ╔═╡ 058a05e0-4d16-457e-88b0-d3cf614c695d
train_set, test_set = partition(eachindex(Y), 0.85; rng=Xoshiro(456))

# ╔═╡ 50db3c8c-c97f-4a06-81a4-556dc379837c
X_train = Float32.(Matrix(X_trans[train_set,:]))

# ╔═╡ 7e772d60-fe7d-4d23-96c9-696cf9b21a93
X_test = Float32.(Matrix(X_trans[test_set,:]))

# ╔═╡ d54cb8c5-60ca-428a-bac5-4466d8660c4e
Y_train = Float32.(Vector(Y[train_set]))

# ╔═╡ 9442ad60-df98-4888-a584-701519eef959
Y_test = Float32.(Vector(Y[test_set]))

# ╔═╡ 5b7c5ddd-f05f-4235-ae3f-c4cd28ad3e6e
train_loader = Flux.DataLoader((X_train', Y_train'), batchsize=32, shuffle=true)

# ╔═╡ adcc515b-fb98-4d90-9041-7b0f318f78cf
md"## Build Model"

# ╔═╡ a59118de-d251-469c-af23-8fe352274af9
snet_model1 = Chain(Dense(4, 32, relu), Dense(32,2, relu), Dense(2,1, sigmoid))

# ╔═╡ 54050a17-7af4-4683-99ff-1c6548694118
l1_reg(x) = sum(x.^2)

# ╔═╡ 31d18fc6-0de5-4beb-a011-422ff1e7c087
function l1_loss(m, x, y)
	the_loss = Flux.Losses.crossentropy(m(x), y)
	for p in Flux.params(m)
		the_loss += sum(l1_reg.(p))
	end
	return the_loss
end

# ╔═╡ bdf48085-80f6-4bf3-91b5-e383f0cd58bc
function l11_loss(m, x, y)
	return Flux.Losses.crossentropy(m(x), y)
end

# ╔═╡ e9e66ef9-ae9b-42cb-bd12-c5539616102c
l11_loss(snet_model1, X_train', Y_train')

# ╔═╡ 6bf3b35f-c461-4849-9377-b74b76cc35cd
l11_loss(snet_model1, train_loader.data[1], train_loader.data[2])

# ╔═╡ fb3819db-b349-448d-90bf-1b8f79c1a04a
the_opt = Flux.setup(Adam(), snet_model1)

# ╔═╡ 75d818d6-117d-4340-9f96-bf0bc9d9230d
begin
    training_loss = Float64[]
    epochs = Int64[]
    for epoch in 1:300
		Flux.train!(l11_loss, snet_model1, train_loader, the_opt)
        push!(epochs, epoch)
        push!(training_loss, l11_loss(snet_model1, train_loader.data[1],train_loader.data[2]))
    end
end

# ╔═╡ c3c3a557-bfe8-46ae-ac8e-3d03cab2e4d3
mapping(epochs, training_loss) * visual(Lines) |> draw

# ╔═╡ 2158c7a8-31e9-4912-91c5-ae28abe31a73
pred_test = snet_model1(X_test')

# ╔═╡ 35ee4cb2-70a3-4960-bb6c-5bf909680430
function get_predicted_classes(preds, threshold)
	return preds .>= threshold 
end

# ╔═╡ 3c45bdfa-f865-4bd0-a0b5-bc0b38a97696
pred_test_classes = get_predicted_classes(pred_test, 0.7)

# ╔═╡ 79912615-778b-4d0a-86c3-b27472b80027
cm1 = confmat(Y_test, Flux.onecold(pred_test_classes))

# ╔═╡ 0882fb95-361c-4081-ba91-f13274c1533f
accuracy(cm1)

# ╔═╡ 3f82c855-d2d4-434e-b377-14bb806f9153
md"## NeuralNetworkBinaryClassifier"

# ╔═╡ 605f94a7-87fc-4101-8b9f-07c1b0a4fa14
Y11 = ads_df1[:, :Purchased]

# ╔═╡ 96b6260d-6245-4264-9d47-6a80e68e8fe6
Y1 = categorical(Y11)

# ╔═╡ 5821f7d4-f4ef-4a0a-bd12-4c5bf673c47c
Y1_train = Y1[train_set]

# ╔═╡ 8ee901e0-c1b4-4e36-8a5a-7d611363bd1a
Y1_test = Y1[test_set]

# ╔═╡ 8952829a-4ed6-4dd4-be58-ff5887fa52d9
sn_builder = MLJFlux.@builder begin
    Chain(
        Dense(4, 32, relu),
        Dense(32, 2, relu),
        Dense(2, 1, sigmoid),
...
# ╔═╡ ed08efde-0399-47a2-93a9-95144ab40698
NeuralNetworkBinaryClassifier = @load NeuralNetworkBinaryClassifier pkg=MLJFlux

# ╔═╡ bcf0cf12-eabc-496e-ae86-889094eabf71
snet_model2 = NeuralNetworkBinaryClassifier(builder=sn_builder)

# ╔═╡ 8b5f5e56-9058-4a4f-aee5-d2c81ee78034
snet_mach = machine(snet_model2, X_trans, Y1)

# ╔═╡ aa6c98ff-5f7b-4f8f-a397-1ca6efdace7f
MLJ.fit!(snet_mach)

# ╔═╡ 62929160-7c48-44e2-bace-1cd57e8b4d42
report(snet_mach)

# ╔═╡ 5335ec2c-54ef-46f3-a13c-2908e798541e
mapping(1:11, report(snet_mach).training_losses) * visual(Lines) |> draw

# ╔═╡ b1f43a55-44d0-4e47-b888-c4c11b2bb824
cross_entropy(predict(snet_mach, X_train), Y1_train)

# ╔═╡ e4c7157b-4d51-48ed-8033-8adfefce8342
pred_test2 = predict(snet_mach, X_test)

# ╔═╡ c287a241-f98b-4879-af9a-78710207ed8c
function get_prediction_class(the_preds::UnivariateFiniteVector{Multiclass{2}, Int64, UInt32, Float32})
	pred_target = Vector{Int64}()
	for cur_i in eachindex(the_preds)
		if pdf(the_preds[cur_i], 0) >= pdf(the_preds[cur_i], 1)
			push!(pred_target, 0)
		else
			push!(pred_target, 1)
		end
...
# ╔═╡ bfcf49e8-bd2a-4a14-8a06-d524169299c6
pred_test_cls2 = get_prediction_class(pred_test2)

# ╔═╡ d3bbaec0-3cd4-4480-8ed4-34ebac0cd206
cm2 = confusion_matrix(pred_test_cls2, Vector{Int64}(Y1_test))

# ╔═╡ cb082019-f0c1-4408-b90b-9dee4c85ffc9
accuracy(cm2)

# ╔═╡ d6f1965d-82ec-4e9e-8b1d-e3d725256bf7
r_epc = range(snet_model2, :epochs, lower=1, upper=300, scale=:log10)

# ╔═╡ c084a301-2441-4e28-a92c-8f5fd2d44b99
lc = learning_curve(
    snet_model2,
    X_trans,
    Y1,
...
# ╔═╡ 4de88ba6-9202-46e4-a520-47743b93bf80
mapping(lc.parameter_values, lc.measurements) * visual(Lines) |> draw

# ╔═╡ 282bca1a-03a0-4906-bb8b-4369b5fb6ec8
tuned_sn_model = TunedModel(model=snet_model2, ranges=[r_epc], measure=cross_entropy)

# ╔═╡ bbb9463f-8fe7-4650-b887-721d4d613c2f
snet_mach2 = machine(tuned_sn_model, X_trans, Y1)

# ╔═╡ 624ebc5c-af00-4c38-8596-04882dd1d0bf
MLJ.fit!(snet_mach2, rows=train_set)

# ╔═╡ 5c1081a4-66a8-410a-947a-6d96fcd1388c
pred_test3 = predict(snet_mach2, X_test)

# ╔═╡ 111a8dfd-5aee-440c-80bb-626aeb48fb63
pred_test_cls3 = get_prediction_class(pred_test3)

# ╔═╡ 01e085a5-8139-4f99-8e48-6225aab43038
cm3 = confusion_matrix(pred_test_cls3, Vector{Int64}(Y1_test))

# ╔═╡ 12534df7-583c-426c-9374-3593ce10d40c
accuracy(cm3)

# ╔═╡ 5a3cfc3d-1f64-4714-93a4-3e2be998e861
...
manifest_format = "2.0"
project_hash = "39674221c57369b4b356875080e155bea230f5a9"
